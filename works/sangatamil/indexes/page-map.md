# பக்க வரைபடம் — சங்கத் தமிழ்

This is the current physical scan/status map for the supplied source.

## Source boundary

The actual supplied PDF has been independently confirmed as **497 scans**. Scans 493–497 render successfully from the same mounted PDF and scan 497 is the back cover.

- canonical source range: **1–497**;
- the earlier 150-page count was a preview-service limitation and is retired;
- do not create scan 498+ records;
- printed-page numbering must be read from the source and never inferred from scan arithmetic.

## Current map

| Scan(s) | Printed page(s) | Source role | Current state |
|---:|---|---|---|
| 1 | — | illustrated cover | `verified` |
| 2 | — | title page | `verified` |
| 3 | — | later library/ownership sticker | `verified` |
| 4–5 | — | ruled blank leaves | `verified` |
| 6 | — | internal title page | `verified` |
| 7 | — | publication information | `partial` |
| 8 | III | handwritten `முன்னுரை` facsimile | `partial` |
| 9 | IV | `ஈ. இராமலிங்கனர் மடல்!` | `verified`; visual fidelity `verified` |
| 10–13 | V–VIII | `அணிந்துரை` | `verified`; visual fidelity `verified` |
| 14 | IX | `பதிப்புரை` | `verified`; visual fidelity `verified` |
| 15 | — | blank leaf | `verified` |
| 16 | — | internal `சங்கத் தமிழ்` title leaf | `verified` |
| 17–19 | printed 2–4 | `மலர்மாரி பொழிகின்றேன்!` | **section complete / verified** |
| 20–24 | printed 5–9 with scan-22 illustration | `யாதும் ஊரே; யாவரும் கேளிர்!` | **section complete / verified** |
| 25–30 | printed 10–15 with scan-26 illustration | `மானங்காத்த மறவன்!` | **section complete / verified** |
| 31–36 | printed 16–21 with scan-32 illustration | `துணை நின்றார் தோழி!` | **section complete / verified after corrective re-transcription and independent second textual + visual-fidelity gate** |
| 37–41 | printed 22–26 with scan-38 illustration | `சுமந்தவன் சுமந்த சோகம்!` | scans 37 and 39 text `verified`; scan 38 illustration `verified`; scans 40–41 text `needs-review`; processing one source page per iteration |
| 42–45 | printed 27–30 with scan-44 illustration | `பாவை புகழ்ந்த பன்றி` | section start verified; current inspected extent only; text pages `needs-review`; scan 44 illustration `verified`; end boundary pending |
| 46–497 | later body / end matter | subsequent source-order sections, citations, illustrations and final back cover | progressive discovery/transcription pending |

## Important controls

- The first visible Arabic printed body number is **2** on scan 17; printed page 1 is not inferred.
- Decorative source headings establish section starts; section ends are confirmed by the next heading or a stronger printed marker.
- Illustration pages are physical records, not omissions.
- Textual fidelity and meaningful visual text fidelity are both required before a text page becomes `verified`.
- Printed Sangam provenance is mirrored in `source-citation-register.md` only after direct source inspection; `verified` is assigned only after the page verification gate.
- Source-visible quoted forms are preserved even when another edition may differ.

## Completed gates

- front-matter printed-text gate: **scans 9–14 / IV–IX — COMPLETE**;
- `மலர்மாரி பொழிகின்றேன்!`: **scans 17–19 — COMPLETE / VERIFIED**;
- `யாதும் ஊரே; யாவரும் கேளிர்!`: **scans 20–24 — COMPLETE / VERIFIED**;
- `மானங்காத்த மறவன்!`: **scans 25–30 — COMPLETE / VERIFIED**;
- `துணை நின்றார் தோழி!`: **scans 31–36 — COMPLETE / VERIFIED after corrective audit**. The earlier weak placeholder/inaccurate records were replaced, then separately rechecked against the source. Scan 36 now carries verified `ஐங்குறுநூறு : பாடல் : 180` / `பாடியவர் : அம்மூவனார்` provenance.

## Active gates

`சுமந்தவன் சுமந்த சோகம்!` — scans **37–41 / printed 22–26**: process **one source page per iteration**. Scans **37 / printed 22** and **39 / printed 24** have now been fully transcribed and directly verified for textual + meaningful visual fidelity; scan 38 remains a verified illustration. The **next text-page iteration is scan 40 / printed 25**. Scan 41 remains untouched by the scan-39 iteration. Scan 41 visibly carries `புறநானூறு : பாடல் : 286` / `பாடியவர் : ஔவையார்`; canonical citation promotion must wait until scan 41 itself passes verification.

`பாவை புகழ்ந்த பன்றி` — begins at **scan 42 / printed 27**. Physical records now exist through scan **45 / printed 30**, including the separately preserved scan-44 illustration. The text records require source audit, and the section end remains intentionally open until scan 46 onward can be directly inspected for the next decorative heading.
